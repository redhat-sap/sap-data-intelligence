...
The ideal solution is to pre-load these modules during node’s startup. When not feasable, this
...
|`ubi-build` | link:./ocp-tempate.json | (_recommended_, _connected_, _default_) To be used in connected OCP
clusters. A local build of SDI Observer image will be performed using UBI8 as the base image.

|`ubi-prebuilt` | link:./ocp-prebuilt-image-template.json | _disconnected_ To be used in
disconnected/offline/air-gapped OCP slusters. The image must be first mirrored to a local
registry.

|`custom-build` | link:./ocp-custom-source-image-template.json.orig | For non-production, it is
possible to use a custom base image (e.g. centos). A very limited or no support will be offered in
case of issues though.

The flavour shall be set in the `run-observer-template.sh` script described below.
...
| `NAMESPACE`         | `sdi-observer`  | Kubernetes namespace where SDI Observer runs or will be running.
...
The observer will only output what would have been done. Use `oc logs -n $NAMESPACE -f
dc/sdi-observer` to monitor its progress.
