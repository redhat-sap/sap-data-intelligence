...
+
2. Execute the same steps as outlined in xref:ubi-build-tmpl-run[] while making sure that in step 2,
...
....
+
3. Execute the same steps as outlined in xref:ubi-build-tmpl-run[] while making sure that in step
   2, `FLAVOUR` is set to the `ocp-prebuilt` and `IMAGE_PULL_SPEC` to your
   `local.image.registry:5000`
...
+
4. Continue with the xref:#run-observer-template[regular template instantiation from step 3
   onward].
