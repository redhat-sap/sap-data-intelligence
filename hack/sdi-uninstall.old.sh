...
function forceDeleteResource() {
    ...
    local nm="$2"
    local name="$3"
    oc patch -n "$nm" "$crd/$name" -p '{"metadata":{"finalizers":null}}'
    oc delete --timeout 21s --wait -n "$nm" "$crd/$name" ||:
    if ! oc get -n "$nm" "$crd/$name" >/dev/null 2>&1; then
        return 0
    fi
    oc delete --timeout 21s --wait --force --grace-period=0s -n "$nm" "$crd/$name"
}
...
function deleteCRD() {
    ...
    if [[ "${#resources[@]}" -gt 1 || ( "${#resources[@]}" == 1 && -n "${resources[0]:-}" ) ]]; then
        for rsnm in "${resources[@]}"; do
            ...
            parallel --semaphore --id "del-$crd" oc delete --timeout 21s --wait -n "$nm" "$name"
        done
